...
@ManagedBean(name = "mbbandejarevisionotros")
@ViewScoped
public class MBBandejaRevisionOtros implements Serializable {
	...
	@Getter @Setter private int indicadorTipoBusqueda;
	...
	public void inicio() {
		...
		oEMensaje = UManejadorUrl.validarUrl("BandejaRevisionOtros.xhtml");
	...
	public void obtenerEstado(){
        if (codigoEstado == UEstado.PENDIENTEDEREGISTRO || codigoEstado == UEstado.PENDIENTEDEEVALUACION || codigoEstado == UEstado.ENPROCESODEEVALUACION || codigoEstado == UEstado.OBSERVADO || codigoEstado == UEstado.AUTORIZADO) {
        	indicadorTabSeleccion = 0;
		}else if (codigoEstado == UEstado.CANCELADO || codigoEstado == UEstado.CERRADO || codigoEstado == UEstado.DESAPROBADO) {
	...
	public void visualizarSolicitudDetalle(ERevisionSolicitud oERevisionSolicitudItem) {
		if (oERevisionSolicitudItem != null) {
			oERevisionSolicitudData.setDescripcionNivel(oERevisionSolicitudItem.getDescripcionNivel());
			oERevisionSolicitudData.setDescripcionEstado(oERevisionSolicitudItem.getDescripcionEstado());
			oERevisionSolicitudData.setDescripcionUsuarioSesion(oERevisionSolicitudItem.getDescripcionUsuarioSesion());
		}
	}
	*/
	...
	public void listarSolicitud() {
		...
		lstRevisionSolicitud = oBORevision.listarSolicitud(oERevisionSolicitud, 3, indicadorTipoBusqueda);
		
		if (lstRevisionSolicitud != null){
			lstRevisionSolicitudPendiente = new ArrayList<ERevisionSolicitud>();
	        for (ERevisionSolicitud obj: lstRevisionSolicitud) {
	            if (obj.getCodigoEstado() == UEstado.PENDIENTEDEREGISTRO || obj.getCodigoEstado() == UEstado.PENDIENTEDEEVALUACION || obj.getCodigoEstado() == UEstado.ENPROCESODEEVALUACION || obj.getCodigoEstado() == UEstado.OBSERVADO || obj.getCodigoEstado() == UEstado.AUTORIZADO) {
	        ...
	        lstRevisionSolicitudHistorico = new ArrayList<ERevisionSolicitud>();
	        for (ERevisionSolicitud obj: lstRevisionSolicitud) {
	            if (obj.getCodigoEstado() == UEstado.CANCELADO || obj.getCodigoEstado() == UEstado.CERRADO || obj.getCodigoEstado() == UEstado.DESAPROBADO) {
