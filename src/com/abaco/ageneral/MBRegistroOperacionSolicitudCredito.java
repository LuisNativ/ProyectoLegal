...
public class MBRegistroOperacionSolicitudCredito implements Serializable {
			...
			if(UAccionExterna.NUEVO == accionExterna){
				...
				*/
				...
				if (UManejadorSesionWeb.obtieneVariableSesion(UVariablesSesion.FICHA_PARAMETRO) != null) {
					...
					oEClienteAdicionalData = oBOCliente.buscarClienteAdicional(oEOperacionSolicitudLoad.getCodigoClientePersona(), codigoTipoCliente);
					...
					if(oEClienteAdicionalData != null){
						oEEvaluacionSolicitudCreditoLegalData.setCodigoTipoPersonaJuridica(oEClienteAdicionalData.getCodigoTipoPersonaJuridica());
						oEEvaluacionSolicitudCreditoLegalData.setMontoCapitalSocialRegistroPublicos(oEClienteAdicionalData.getMontoCapitalSocialRegistroPublicos());
						oEEvaluacionSolicitudCreditoLegalData.setMontoCapitalSocialActual(oEClienteAdicionalData.getMontoCapitalSocialActual());
						oEEvaluacionSolicitudCreditoLegalData.setCodigoFacultadOperar(oEClienteAdicionalData.getCodigoFacultadOperar());
						oEEvaluacionSolicitudCreditoLegalData.setCodigoTipoSuscripcionPago(oEClienteAdicionalData.getCodigoTipoSuscripcionPago());
						oEEvaluacionSolicitudCreditoLegalData.setNumeroSuscripcionPago(oEClienteAdicionalData.getNumeroSuscripcionPago());
						oEEvaluacionSolicitudCreditoLegalData.setNumeroAcciones(oEClienteAdicionalData.getNumeroAcciones());
						oEEvaluacionSolicitudCreditoLegalData.setIndicadorAvalarTercero(oEClienteAdicionalData.getIndicadorAvalarTercero());
						oEEvaluacionSolicitudCreditoLegalData.setIndicadorGrabarBien(oEClienteAdicionalData.getIndicadorGrabarBien());
						oEEvaluacionSolicitudCreditoLegalData.setDescripcionAvalarTercero(oEClienteAdicionalData.getDescripcionAvalarTercero());
						oEEvaluacionSolicitudCreditoLegalData.setObservacionAvalarTercero(oEClienteAdicionalData.getObservacionAvalarTercero());
						oEEvaluacionSolicitudCreditoLegalData.setObservacionGrabarBien(oEClienteAdicionalData.getObservacionGrabarBien());
						oEEvaluacionSolicitudCreditoLegalData.setObservacionConstanciaSocial(oEClienteAdicionalData.getObservacionConstanciaSocial());						
					...
					if(oEUsuario.getCodigoArea() == UArea.LEGAL){
						...
						*/
						//if(oEOperacionSolicitudLoad.getCodigoAutorizaJefe() == UEstadoAutorizacionJefe.AUTORIZADOCOMPLETADO){
						if(oEOperacionSolicitudLoad.getCodigoAutorizaJefe() == UEstadoAutorizacionJefe.AUTORIZADO && oEOperacionSolicitudLoad.getCodigoEstado() != UEstado.OBSERVADO){
							codigoEstado = oEOperacionSolicitudLoad.getCodigoEstado();
						}else if(oEOperacionSolicitudLoad.getCodigoAutorizaJefe() == UEstadoAutorizacionJefe.AUTORIZADO && oEOperacionSolicitudLoad.getCodigoEstado() == UEstado.OBSERVADO){
							codigoEstado = UEstado.ENEVALUACION;
							visualizarBtnGrabar = true;
						}else if(oEOperacionSolicitudLoad.getCodigoAutorizaJefe() == UEstadoAutorizacionJefe.SOLICITADO){
							codigoEstado = UEstado.ENEVALUACION;
						}else {
							codigoEstado = UEstado.ENEVALUACION;
							visualizarBtnGrabar = true;
	...
	public void guardar() {
		...
		generarLogMovimiento(oEEvaluacionSolicitudCreditoLegal);
		...
		if(UAccionExterna.NUEVO == accionExterna){
			oEMensaje = oBOOperacion.modificarEvaluacionSolicitudCredito(oEOperacionSolicitud, oEEvaluacionSolicitudCreditoLegal, oEClienteData, oEClienteConstitucionEmpresaData, oEClienteAdicionalData, codigoTipoCliente);
		}else if(UAccionExterna.EDITAR == accionExterna){
			oEMensaje = oBOOperacion.modificarEvaluacionSolicitudCredito(oEOperacionSolicitud, oEEvaluacionSolicitudCreditoLegal, oEClienteData, oEClienteConstitucionEmpresaData, oEClienteAdicionalData, codigoTipoCliente);
		}
	...
	public void generarLogMovimiento(EEvaluacionSolicitudCreditoLegal oEEvaluacionSolicitudCreditoLegal) {
		int correlativo = 0;
		...
		if(!oEEvaluacionSolicitudCreditoLegalLoad.getNombreLargo().equals(oEEvaluacionSolicitudCreditoLegal.getNombreLargo())
			){
			oESolicitudLogMovimiento = new ESolicitudLogMovimiento();
			oESolicitudLogMovimiento.setCodigoOrden(correlativo +1);
	...
	*/
	...
	public void validarTamanioDocumentoContratanteConyugue(){
		if(oEEvaluacionSolicitudCreditoLegalData.getCodigoTipoDocumentoConyugue() != null){
			if(oEEvaluacionSolicitudCreditoLegalData.getCodigoTipoDocumentoConyugue().equals(UTipoDocumento.RUC)){
				maxLgnNumeroDocumentoContratanteConyugue = UMaximoTamanio.RUC_MAXLGN;
			}else if(oEEvaluacionSolicitudCreditoLegalData.getCodigoTipoDocumentoConyugue().equals(UTipoDocumento.DNI) || oEEvaluacionSolicitudCreditoLegalData.getCodigoTipoDocumentoConyugue().equals(UTipoDocumento.LIBRETA_ELECTORAL)){
	...
	public void obtenerTipoDocumentoContratanteConyugue() {
		validarTamanioDocumentoContratanteConyugue();
		oEEvaluacionSolicitudCreditoLegalData.setDocumentoConyugue("");
	}
	...
	*/
	...
	public void listarClienteHistorico(){
		lstClienteHistorico = oBOCliente.listarClienteHistorico(codigoTipoCliente, oEEvaluacionSolicitudCreditoLegalData.getCodigoCliente());
	}
	...
	public void imprimirDocumento2() {
        ...
        */
		...
		for(int i=0;i<lstAval.size();i++){
			...
			oTercero = oBOSolicitudCredito.buscarAval(lstAval.get(i).getCodigoAval(), oEEvaluacionSolicitudCreditoLegalLoad.getNumeroSolicitud());
			...
			correlativoAval = i+1;
			obj.setCargoLaboral("AVAL "+ lstAval.get(i).getCodigoAval());
	...
	public void listarSolicitudesCredito(){
		...
		lstEvaluacionSolicitudCreditoLegal = oBOSolicitudCredito.listarEvaluacionSolicitudCreditoLegal(oEEvaluacionSolicitudCreditoLegalLoad.getCodigoCliente(), oEEvaluacionSolicitudCreditoLegalLoad.getNumeroDocumento());
	}
